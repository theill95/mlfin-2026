...
with title= hover docs, task -> work cell (blank-safe `...`) -> 1-2 folded hints
...
'<code style="cursor:help" title="Move a column down by n rows. shift(1) is yesterday\'s value on today\'s row.">s.shift(1)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Move a column UP: shift(-1) puts tomorrow\'s value on today\'s row. This is how a target is built.">s.shift(-1)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A moving window of n rows. Follow it with .mean(), .std() or .sum().">s.rolling(20)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Build a table out of named columns. Series are lined up on their labels for you.">pd.DataFrame({\'a\': s1, \'b\': s2})</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Turn True and False into 1 and 0.">(s &gt; 0).astype(int)</code></p>\n\n'
'<p style="line-height:2.1"><strong>Missing values</strong><br>\n'
'<code style="cursor:help" title="Force a table onto a different set of row labels. Rows that did not exist arrive empty.">frame.reindex(index)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Every calendar day between two dates.">pd.date_range(start, end, freq=\'D\')</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="True wherever the value is missing.">frame.isna()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Count the missing values in each column.">frame.isna().sum()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Throw away rows with missing values.">frame.dropna()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Fill missing values with something you choose.">s.fillna(value)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Carry the last known value forward. It only ever looks backwards, which is what forecasting needs.">s.ffill()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Draw a straight line across the gap. Careful: it uses the value AFTER the gap.">s.interpolate()</code></p>\n\n'
'<p style="line-height:2.1"><strong>Extreme values</strong><br>\n'
'<code style="cursor:help" title="Count, mean, standard deviation, min, quartiles and max, all at once.">s.describe()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The value below which that fraction of the data lies. Pass a list to get several at once.">s.quantile([0.25, 0.75])</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Absolute value, element by element.">s.abs()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Combine two conditions: and, or. Each side needs its own brackets.">(a) &amp; (b)  ·  (a) | (b)</code></p>\n\n'
'<p style="line-height:2.1"><strong>Transformations</strong><br>\n'
'<code style="cursor:help" title="The natural logarithm, element by element.">np.log(values)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The difference between each row and the one before it.">s.diff()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Standardise: subtract the mean, divide by the standard deviation.">(s - s.mean()) / s.std()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="One 0/1 column per category. This is how text gets into a feature table.">pd.get_dummies(frame, columns=[\'sector\'], dtype=int)</code></p>\n\n'
'<p style="line-height:2.1"><strong>Scoring a prediction</strong><br>\n'
'<code style="cursor:help" title="Mean absolute error: the average size of a mistake, in the units of the target.">(y - yhat).abs().mean()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Mean squared error: punishes a big miss far harder than several small ones.">((y - yhat) ** 2).mean()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Root mean squared error: back in the units of the target.">np.sqrt(mse)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="How much better than always predicting the average. Zero means no better at all.">1 - rss / tss</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Count the days where both conditions hold. This is one cell of a confusion matrix.">(said &amp; happened).sum()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The opposite of a boolean Series: True becomes False.">~said</code></p>\n\n'
'<p style="line-height:2.1"><strong>Splitting, and measuring flexibility</strong><br>\n'
'<code style="cursor:help" title="Rows up to a date, and rows from a date. A DataFrame with a date index slices with dates.">frame.loc[:\'2022-12-31\']  ·  frame.loc[\'2023-01-01\':]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Fit a polynomial of the given degree and hand back its coefficients.">np.polyfit(x, y, degree)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Evaluate a fitted polynomial at some x values.">np.polyval(coef, x)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="An array of the same value, repeated. Useful as a baseline prediction.">np.full(n, value)</code></p>\n\n'
'<p style="line-height:2.1"><strong>Still yours from Session 3</strong><br>\n'
'<code style="cursor:help" title="Make a figure and one axes to draw on. Every plot starts here.">fig, ax = plt.subplots(figsize=(9, 3))</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A line: something over time.">ax.plot(x, y, label=name)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A scatter: one thing against another.">ax.scatter(x, y, s=10)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A horizontal reference line, for example at zero.">ax.axhline(0)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Title and axis labels. Always say what the numbers are.">ax.set_title(t, loc=\'left\')</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Show the legend. Needs label= on each thing you drew.">ax.legend()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A log scale, for values that span orders of magnitude.">ax.set_yscale(\'log\')</code></p>'
)
