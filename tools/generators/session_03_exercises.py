...
'<p style="line-height:2.1"><strong>NumPy arrays</strong><br>\n'
'<code style="cursor:help" title="Turn a list of numbers into an array, which does arithmetic on every element at once.">np.array(list)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="How many rows and columns. For a 1-D array it prints as (n,).">values.shape</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Everything from position 1 onwards. Same slicing as a list.">values[1:]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Everything except the last element.">values[:-1]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The square root, element by element.">np.sqrt(x)</code></p>\n\n'
'<p style="line-height:2.1"><strong>Summarising an array</strong><br>\n'
'<code style="cursor:help" title="The average of the values.">values.mean()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The standard deviation, which is what volatility is.">values.std()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The smallest value.">values.min()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The largest value.">values.max()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Add every value up.">values.sum()</code></p>\n\n'
'<p style="line-height:2.1"><strong>Asking a question of every element</strong><br>\n'
'<code style="cursor:help" title="An array of True and False, one per element.">values &gt; 0</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Keep only the elements where the condition is True.">values[values &gt; 0]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Count the Trues, because True counts as 1.">(values &gt; 0).sum()</code></p>\n\n'
'<p style="line-height:2.1"><strong>Matrices</strong><br>\n'
'<code style="cursor:help" title="A 2-D array: a list of rows, each row a list of numbers.">np.array([[1, 2], [3, 4]])</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="One element: row 0, column 1. Rows first, then columns.">X[0, 1]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A whole row, or a whole column. The colon means all of them.">X[0, :]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The transpose: rows become columns.">X.T</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Matrix multiplication. For two vectors this is the dot product: multiply pairwise, then add.">A @ B</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A column of ones, n long. Useful as the intercept column of a regression.">np.ones(n)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Glue 1-D arrays together side by side, as columns of a matrix.">np.column_stack([a, b])</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The inverse: the matrix that undoes A.">np.linalg.inv(A)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Solve A x = b for x. Safer and faster than inverting.">np.linalg.solve(A, b)</code></p>\n\n'
'<p style="line-height:2.1"><strong>pandas: one column</strong><br>\n'
'<code style="cursor:help" title="A Series: values with a label on each one. Build it from a dictionary or a list.">pd.Series(data)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Look a value up by its label, like a dictionary.">s[\'AAPL\']</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Sort by value. Pass ascending=False for largest first.">s.sort_values()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A Series summarises itself, exactly like an array.">s.mean()</code></p>\n\n'
'<p style="line-height:2.1"><strong>pandas: a table</strong><br>\n'
'<code style="cursor:help" title="Read a CSV file into a DataFrame. parse_dates turns a text date column into real dates.">pd.read_csv(path, parse_dates=[\'date\'])</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The first n rows, 5 by default.">frame.head(n)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Rows and columns, as a pair of numbers.">frame.shape</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="What each column holds: dates, text, numbers.">frame.dtypes</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The column names.">frame.columns</code></p>\n\n'
'<p style="line-height:2.1"><strong>pandas: choosing what you want</strong><br>\n'
'<code style="cursor:help" title="One column, as a Series.">frame[\'close\']</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Several columns, as a smaller table. Note the two sets of brackets.">frame[[\'date\', \'close\']]</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Keep only the rows where the condition is True.">frame[frame[\'ticker\'] == \'AAPL\']</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Take your own copy, so pandas knows you mean to change this selection and not the original.">.copy()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Rows by label. With a date index you can slice with dates.">frame.loc[\'2024\']</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Sort the rows by the values in a column.">frame.sort_values(\'ret\')</code></p>\n\n'
'<p style="line-height:2.1"><strong>pandas: new columns and groups</strong><br>\n'
'<code style="cursor:help" title="Create a column by assigning to a name that does not exist yet.">frame[\'ret\'] = ...</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The percentage change from each row to the next. The first one is NaN.">frame[\'close\'].pct_change()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Throw away the rows with no value (NaN).">s.dropna()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Split by the values in a column, do something inside each group, put the answers back together.">frame.groupby(\'ticker\')</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Several summaries at once, as a table.">g.agg([\'mean\', \'std\'])</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Reshape: one row per date, one column per ticker.">frame.pivot(index=\'date\', columns=\'ticker\', values=\'close\')</code></p>\n\n'
'<p style="line-height:2.1"><strong>Building tables out of what you have</strong><br>\n'
'<code style="cursor:help" title="A Series straight from a dictionary: the keys become the labels.">pd.Series(a_dict)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A DataFrame from a dictionary of columns. Series are lined up on their labels for you.">pd.DataFrame({\'vol\': s1, \'ret\': s2})</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A DataFrame from a list of dictionaries: one row per dictionary. Useful when you build rows in a loop.">pd.DataFrame(list_of_dicts)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The plain numbers underneath, as a NumPy array, with the labels dropped.">frame.values</code></p>\n\n'
'<p style="line-height:2.1"><strong>Still yours from Sessions 1 and 2</strong><br>\n'
'<code style="cursor:help" title="Run the indented body once for every item.">for item in items:</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Run the block only when the condition is True. elif adds further cases, else catches the rest.">if / elif / else</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Define your own function. The indented body is what it does, and return hands a value back.">def name(x, rate=0.05):</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Hand a value back to whoever called the function.">return value</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="An empty list, ready to be filled by a loop, and the method that fills it.">results = []  ·  results.append(x)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A dictionary: values stored under a name rather than a position.">{\'AAPL\': 0.36}</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Show r as a percentage with 2 decimals.">f\"{r:.2%}\"</code></p>\n\n'
'<p style="line-height:2.1"><strong>matplotlib</strong><br>\n'
'<code style="cursor:help" title="Make a figure and one axes to draw on. figsize is in inches.">fig, ax = plt.subplots(figsize=(9, 3))</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A line: something over time.">ax.plot(x, y)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Horizontal bars: comparing named things.">ax.barh(names, values)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A histogram: the shape of one variable.">ax.hist(values, bins=40)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="A scatter: one thing against another.">ax.scatter(x, y)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="The title. loc=\'left\' puts it on the left, which reads better.">ax.set_title(text, loc=\'left\')</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Label the axes. Always say what the numbers are, and in what units.">ax.set_ylabel(text)</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Show the legend. Needs label= on each line you drew.">ax.legend()</code> &nbsp;&nbsp; '
'<code style="cursor:help" title="Save the figure to a file.">fig.savefig(\'name.png\', dpi=200)</code></p>'
)
