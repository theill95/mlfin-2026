...
# -*- coding: utf-8 -*-
"""Cumulative check for the Session 6 case: run the quick load, then exec every
...
os.chdir(str(Path(__file__).resolve().parents[2] / "session_06"))
import matplotlib
matplotlib.use("Agg")

NB = "session_06_case.ipynb"
...
PROMISED = ["table5", "model5", "table", "train", "test", "columns", "one_model",
            "one_rmse", "ols_model", "ols_rmse", "cv_by_alpha", "search", "ridge_rmse",
            "lasso_search", "lasso_rmse", "kept", "enet_search", "enet_rmse",
            "desk", "wins", "calm", "report"]
