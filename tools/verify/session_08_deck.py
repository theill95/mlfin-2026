...
# -*- coding: utf-8 -*-
"""Run every {pyodide} cell of the Session 8 deck, in document order, in ONE
...
os.chdir(str(Path(__file__).resolve().parents[2] / "session_08"))
import matplotlib
matplotlib.use("Agg")
...
# The two "Your turn" cells ship with `...` blanks, so they are meant to fail
# until a student fills them in. The two "predict" cells run clean (they hold
# real code, only autorun is off). Everything else must run clean.
