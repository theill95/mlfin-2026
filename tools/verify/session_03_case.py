...
# -*- coding: utf-8 -*-
"""Cumulative check for the Session 3 case: run the quick load, then exec every
...
os.chdir(str(Path(__file__).resolve().parents[2] / "session_03"))
import matplotlib
matplotlib.use("Agg")

NB = "session_03_case.ipynb"
...
saved = ["riskiest", "calmest"]
print("final saved state:", {k: ns.get(k) for k in saved if k in ns})
print("variables the closing table promises:",
      [v for v in ["prices", "wide", "vol", "annual_vol", "riskiest", "calmest", "report"]
       if v in ns])
