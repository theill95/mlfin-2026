...
# -*- coding: utf-8 -*-
"""Run every {pyodide} cell of the Session 7 deck, in document order, in ONE
...
os.chdir(str(Path(__file__).resolve().parents[2] / "session_07"))
import matplotlib
matplotlib.use("Agg")
...
# The twelve "Your turn" cells ship with `...` blanks, so they are meant to
# fail until a student fills them in. Their answer slides ("Your turn N:
# answer") are static code, except the log-target answer, which is a live cell
# so that the valuation the rest of the deck uses exists without the student's
# cell having been run. Everything else must run clean.
EXPECT_ERROR = ("Your turn",)
RUNS_CLEAN = ("answer",)
...
for kind, title, opts, code in cells:
    if kind != "pyodide":
        continue
    expect_err = any(k in title for k in EXPECT_ERROR) and not any(k in title for k in RUNS_CLEAN)
    out = io.StringIO()
    disp = None
    err = None
    try:
    ...
    if expect_err:
        # a blank your-turn cell may error on its `...` or simply print it;
        # either is fine, as long as it does not display something that looks
        # like an answer (a DataFrame, say) - check that by eye below
        status = "ok  (blank: errors)" if err else "ok  (blank: runs)"
